...
function ajaxCiba() {
    // 清空容器，准备加载新内容
    $("#walBox").html('<div id="loadmore">正在加载每日寄语...</div>');
    
    $.ajax({
        url: "https://v1.hitokoto.cn/?c=k", // 使用 Hitokoto 接口
        type: "GET",
        dataType: "json",
        success: function(data) {
            // 构造 HTML 结构
            // 这里使用了 Bootstrap 的类名来保持风格统一
            var newHtml = `
                <div class="container">
                    <div class="hitokoto-container animate__animated animate__fadeIn">
                        <span id="line-en">${data.hitokoto}</span>
                        <span id="line-cn">—— ${data.from}</span>
                    </div>
                </div>
            `;
            ...
            console.log("渲染成功：" + data.hitokoto);
        },
        error: function() {
            $("#walBox").html('<div class="hitokoto-container">加载失败，请重试。</div>');
        }
    });
    return true;
}
